...
const PizzaBlock = () => {
    ...
    const renderPizza = pizzaList.map(pizza => {
        return (
                ...
                <div className="pizza-block__selector">
                    <ul>
                        {pizza.types.map((type, index) => (
                            <li key={index} className={type === 0 ? "active" : ""}>{type === 0 ? 'тонкое' : 'традиционное'}</li>
                        ))}
                    </ul>
                    <ul>
                        {pizza.sizes.map((size, index) => (
                            <li key={index} className={size === 26 ? "active" : ""}>{size} см.</li>
                        ))}
                    </ul>
                </div>
